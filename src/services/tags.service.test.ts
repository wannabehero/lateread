...
describe("tags.service", () => {
  ...
  describe("getOrCreateTag", () => {
    it("should create a new tag if it does not exist", async () => {
      ...
      const { id: tagId } = await getOrCreateTag(user.id, "new-tag");
    ...
    it("should return existing tag ID if tag exists", async () => {
      ...
      const { id: tagId } = await getOrCreateTag(user.id, "existing-tag");
    ...
    it("should be case-insensitive when checking existing tags", async () => {
      ...
      const { id: tag1Id } = await getOrCreateTag(user.id, "TypeScript");
      const { id: tag2Id } = await getOrCreateTag(user.id, "typescript");
      const { id: tag3Id } = await getOrCreateTag(user.id, "TYPESCRIPT");
    ...
    it("should set autoGenerated flag correctly", async () => {
      ...
      const { id: autoTagId } = await getOrCreateTag(user.id, "auto-tag", true);
      const { id: manualTagId } = await getOrCreateTag(
        user.id,
        "manual-tag",
        false,
      );
    ...
    it("should default to autoGenerated=true", async () => {
      ...
      const { id: tagId } = await getOrCreateTag(user.id, "default-tag");
    ...
    it("should isolate tags between users", async () => {
      ...
      const { id: tag1Id } = await getOrCreateTag(user1.id, "shared-name");
      const { id: tag2Id } = await getOrCreateTag(user2.id, "shared-name");
